...
def main():
    ...
    if params['order']=='test':
        ...
        os.system(cmd)
        os.chdir(testdir)
        test(os.path.basename(params['deploy_file']),modeltopdir,os.path.join(params['model_topdir'],params['predict_filelist']),int(params['gpunum']),int(params['trial_num']),testdir,params['optimwrt'],params['outputlayer'])
        flag = True
